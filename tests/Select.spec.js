...
describe('Select', () => {
  describe('render', () => {
    ...
    it('renders default value correctly', () => {
      const wrapper = render(
        <Select defaultValue={castValue('2')}>
  ...
  it('convert value to array', () => {
    const wrapper = mount(
      <Select value={castValue('1')}>
        <OptGroup>
          <Option value={castValue('1')} title="一">1</Option>
        </OptGroup>
      </Select>
    );
  ...
  it('convert defaultValue to array', () => {
    const wrapper = mount(
      <Select defaultValue={castValue('1')}>
        <OptGroup>
          <Option value={castValue('1')} title="一">1</Option>
        </OptGroup>
      </Select>
  ...
  it('should show selected value in singleMode when close', () => {
    ...
    expect(wrapper.find('.rc-select-selection-selected-value').props().children).toBe(castValue(1));
  });
  ...
  it('filter options by values', () => {
    ...
    expect(dropdownWrapper.find('MenuItem').props().value).toBe(castValue('1'));
  });

  it('specify which prop to filter', () => {
    const wrapper = mount(
      <Select optionFilterProp="label">
        <Option value={castValue('1')} label="One">1</Option>
        <Option value={castValue('2')} label="Two">2</Option>
      </Select>
    );
    ...
    expect(dropdownWrapper.find('MenuItem').length).toBe(1);
    expect(dropdownWrapper.find('MenuItem').props().value).toBe(castValue('2'));
  });

  it('no search', () => {
    const wrapper = render(
      <Select showSearch={false} value={castValue('1')}>
        <Option value={castValue('1')}>1</Option>
        <Option value={castValue('2')}>2</Option>
      </Select>
    );
  ...
  it('adds label to value', () => {
    ...
    expect(handleChange).toBeCalledWith({ key: castValue('1'), label: 'One' });
  });
